...
library(tidyverse)
library(gganimate)
## Get Fira Sans and Fira Mono from: https://www.fontsquirrel.com/fonts/fira-sans
#devtools::install_github("wch/Rttf2pt1@freetype2")
#devtools::install_github("wch/extrafont@freetype")
library(extrafont)
extrafont::font_import(pattern = "Fira", prompt = FALSE)


setwd("~/Dropbox/3 Portfolio/USU/Data Analytics/data_joins_demonstration/")
...
plot_data <- function(x, title = "") {
  ggplot(x) +
    ...
    geom_text(aes(x = .x), hjust = 0.5, size = 12, family = "Fira Mono", color = "white") +
    scale_fill_identity() +
    coord_equal() +
    ggtitle(title) +
    theme_void() +
    theme(plot.title = element_text(family = "Fira Sans", hjust = 0.5, size = 24)) +
    guides(fill = FALSE)
}
...
joined_df <- bind_rows(
  ...
  mutate(frame = 2, .x = .x + 1) %>%
  # mutate(color = case_when(value == "x3" ~ "white",
  #                          value == "y4" ~ "white",
  #                          value == "3" ~ "white",
  #                          value == "4" ~ "white",
  #                          TRUE ~ color)) %>%
...
ij <- initial_dfs %>%
  ...
  arrange(remove, desc(frame), value) %>%
